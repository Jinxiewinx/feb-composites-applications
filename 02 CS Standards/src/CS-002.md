...
| E | 2026-08-29 | Claude | Coupons caught up with the app's v4.0.0 R&D bench (CS-013 Rev F): a coupon is `CPN-SNx-###` under a study that carries the stack and lots, so the filename-to-layup chain now runs through the study record. The write-the-ID-before-the-first-cut rule is unchanged. |
...
![The §7.3 example stack drawn in section. P1 touches the mold and becomes the show face; function plies are drawn, and specified, explicitly.](figures/cs002-stack.png)
...
A coupon is `CPN-SNx-###`: its own record on the app's R&D bench, belonging to a study `RDS-SNx-###` (CS-013 §4.1). The study carries the stack, fabric lots, resin and hardener lots once, and every coupon inherits them unless a coupon overrides its own; that inheritance is what makes one coupon ID enough to recover the whole build. (The retired suffix spelling `PNL-SNx-###-Cnn` survives only on frozen SN5-era records.)
...
2. Name the raw data file after the coupon: `CPN-SN6-042.csv`. That one filename then carries the study, the stack, the session and the material lots, through one link each, none of them retyped.
