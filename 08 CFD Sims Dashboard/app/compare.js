...
export function renderOverlay(main) {
  ...
  async function renderOverlayStage() {
    ...
    if (S.overlay.mode === "blend") {
      ...
      /* Touch: onMove already reads ev.touches, but 07 never registered the
         listeners, so a tablet could not drag the divider at all. Passive is
         off so the drag does not also scroll the page. */
      const tstart = ev => {
        ev.preventDefault();
        onMove(ev);
