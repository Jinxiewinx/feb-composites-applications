...
the free tier backs fourteen tabs (Dashboard, Work Orders, Parts, Stock, Molds,
...
public scan page. This is the day-to-day tool. The full manual, setup, deploy
...
## What the single-file tool does

The list view shows every work order with status and subteam filters plus
search. New Work Order pre-fills the standard step list for the chosen process
type (infusion, wet layup, and so on), including blocker steps like stack
freeze, mold design review and drop test, which hold up later buy-offs until
they're signed.

The detail view carries overview fields, the mold record, a layup-stack table
with a visual, BOM, steps with buy-offs, quality checks with pass/fail,
timeline and notes.
...
The sheet is capped at two pages. `print.js` renders it, measures the result,
and walks down a ladder of progressively tighter layouts until it fits, so the
writing space flexes with how full the work order is instead of the page count
flexing. `tools/print-preview.html` has an Audit all button that runs every
seed work order and every blank form through that loop and reports the page
counts, which is how the cap gets verified after a layout change.
...
`retro`, and unverifiable fields are left blank or say "not recorded (retro)"
by design. They're honest references, not fabricated exemplars. The single-file
version loads only the 26 work orders.

The hosted version alone has accounts and a roster allowlist, roles, live
multi-user sync, shared numbering, buy-offs that record the signed-in member,
photo and file attachments, and everything else in `app/README.md`. In the
offline version, buy-offs are typed initials and data stays in that one
browser.

One honest caveat on hosted buy-offs: they record identity, but they aren't
tamper-proof. Any roster member can edit any work order, and the JSON exports
in Drive are the only version history. Details and mitigations are in
...
browser data, switching browsers or switching computers loses it, so export
JSON weekly if you're actually working out of it (CS-013 §7.4). Import JSON
merges a file back in, overwriting same-ID records and adding new ones.
...
`updatedAt` and `updatedBy` on each record. In the single-file tool, photos are
references only: filenames and captions, with the actual photos in the part's
Drive folder. The hosted app stores real attachments.
...
- The governing standard is CS-013, Work Orders and Part Traceability. The ID grammar for every other prefix (MOLD-, P-, and the rest) is CS-013 §4.1.

## Testing

From `SN6 Resources/`, run `node tools/test_app.mjs` for app logic across all
tabs, no Firebase needed. The full suite, including the browser and emulator
tests, is inventoried in `tools/README.md`.

## Future ideas for the single-file tool (deliberately not built)

Keep this one boring. Multi-user sync, accounts, shared numbering, photo
storage and dashboards all graduated into `app/` during July 2026; that is
where new capability goes. This file's value is that it opens anywhere,
forever, with nothing installed.
