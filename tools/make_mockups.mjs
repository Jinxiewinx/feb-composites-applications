...
const SHOTS = [
  { id: "dashboard", kind: "app", badge: 1, vh: 1000,
    ...
    note: "The board, on the app's own white surfaces: gold-slash Saira headers and big numerals carry the identity. The alert strip is the lead's one-second read and owns the T-minus outright — late, blocked, unassigned, curing. Under it, the bucketed work list (one row per physical run, with a flag where issues are holding one up), Shop status narrowed to what is stopping work right now, build progress by stage, this week's stations, the cross-app activity feed, the milestone and the season streaks, money with the $50 approval rule, a launchpad of filtered jumps, and Stock & housekeeping sunk to the bottom where a monthly habit belongs. Every element still a link." },
  { id: "season", kind: "app", badge: 2, vh: 950,
    ...
    note: "The Master Tracker sheet, in the app. One row per part the team means to make, and most cells empty by design \u2014 a row exists the moment somebody commits to building the thing, and the specifics arrive later. Every cell edits in place; the three stage columns are colour-coded and still go through the evidence gate before a value sticks. The part name stays pinned while the other twelve columns scroll." },
