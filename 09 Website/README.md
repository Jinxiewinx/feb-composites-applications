...
system in `../05 Design System/`. It has never been deployed; see
...
```bash
node "09 Website/build.mjs"          # copies the design system into site/_ds/feb/
python3 -m http.server 8000 --directory "09 Website/site"
```

Then open http://localhost:8000. Run `build.mjs` once after cloning; after that
only when `../05 Design System/` changes. Opening `site/index.html` straight off
...
| `site/_ds/feb/` | **Generated.** The design system, copied from `../05 Design System/` by `build.mjs`. Gitignored. Never edit it here. |
...
The site links `../05 Design System/` rather than carrying its own copy of the
...
there is one source of truth and it lives in `05 Design System/`.
...
- **The `table.list` narrow-screen collapse.** `05 Design System/README.md` says
...
2. `node "09 Website/build.mjs"`
3. `firebase deploy --only hosting` from inside `09 Website/`.

The app at `feb-composites.web.app` deploys separately from `06 Composites App/` and is
unaffected.
