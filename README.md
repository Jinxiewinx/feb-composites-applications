...
![Dashboard: the glance board: hero numbers over five compact modules](03%20App/design/dashboard-mockup-20260804.png)

Eleven tabs, grouped in the sidebar by who is asking (Dashboard and Tickets
up top, then Build, Planning, and Team):

- **Dashboard:** the glance board and the page you land on: a hero band (assigned to you, blocked, late, the money) over five compact modules: the deadline list, the season bars for parts and molds, the booked stations, shop status with severity dots, and new activity. Read-only; fits one projected screen; every element links into the tab it came from.
...
- **Work Orders:** the manufacturing traveler, as a split view like Parts and Molds. The rail indexes every run grouped by the part it builds, so the tab reads as the part → run → mold hierarchy; each row carries how far through its buy-offs it is, and whether it is blocked or curing. Parts nobody has started show up too, with the button that starts one. Group, sort and filter are yours to set. The pane keeps the whole record in one scroll, Steps first because that is the bench action, with a bar above it that jumps to Overview, Stack & BOM, Quality, Files or Notes and carries a count per section plus a dot when something in one needs attention. What is true of the whole record (which run it is, its status, the lineage, and anything blocking it) stays above that bar. Layup stack, BOM, step buy-offs stamped with who signed, blocker steps, and enforced cure holds backed by the datasheets in `resins.js`. Prints to a hand-fillable sheet that is always exactly two pages.
